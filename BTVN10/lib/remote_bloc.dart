...
class RemoteBloc {
  var state = RemoteState(70,0); // init giá trị khởi tạo của RemoteState. Giả sử TV ban đầu có âm lượng 70
  ...
  RemoteBloc() {
    ...
    eventController.stream.listen((RemoteEvent event) {
      ...
      if (event is IncrementEvent) {
        ...
        state = RemoteState(state.volume + event.increment,state.channel);
      } else if (event is DecrementEvent) {
        // xử lý giảm âm lượng
        state = RemoteState(state.volume - event.decrement,state.channel);
      } else if(event is IncrementChannelEvent){
        state = RemoteState(state.volume, state.channel+event.incrementChannel);
      } else if(event is DecrementChannelEvent){
        state = RemoteState(state.volume, state.channel-event.decrementChannel);
      }
      else {
        // xử lý mute
        state = RemoteState(0,state.channel);
      }
